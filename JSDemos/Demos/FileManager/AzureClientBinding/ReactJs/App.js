...
function App() {
  ...
  const getItems = (parentDirectory) => azure.getItems(parentDirectory.path);
  const createDirectory = (parentDirectory, name) =>
    azure.createDirectory(parentDirectory.path, name);
  const renameItem = (item, name) =>
    (item.isDirectory ? azure.renameDirectory(item.path, name) : azure.renameFile(item.path, name));
  const deleteItem = (item) =>
    (item.isDirectory ? azure.deleteDirectory(item.path) : azure.deleteFile(item.path));
  const copyItem = (item, destinationDirectory) => {
    const destinationPath = destinationDirectory.path
  ...
  const moveItem = (item, destinationDirectory) => {
    const destinationPath = destinationDirectory.path
      ? `${destinationDirectory.path}/${item.name}`
      : item.name;
    return item.isDirectory
  ...
  const uploadFileChunk = (fileData, uploadInfo, destinationDirectory) => {
    ...
    if (uploadInfo.chunkIndex === 0) {
      const filePath = destinationDirectory.path
        ? `${destinationDirectory.path}/${fileData.name}`
        : fileData.name;
  ...
  const [fileSystemProvider] = React.useState(
  ...
  return (
    ...
    >
      ...
      <div id="message-box">
        ...
        <a
          href="https://js.devexpress.com/Demos/WidgetsGallery/Demo/FileManager/AzureClientBinding/React/Light/"
          target="_blank"
          rel="noopener noreferrer"
        >
          https://js.devexpress.com/Demos/WidgetsGallery/Demo/FileManager/AzureClientBinding/React/Light/
        </a>
