...
const App = () => {
  ...
  const onDragEnd = (e) => {
    ...
    setItemsDriveC([...itemsDriveC]);
    setItemsDriveD([...itemsDriveD]);
  ...
  const moveNode = (fromNode, toNode, fromItems, toItems, isDropInsideItem) => {
    ...
    const toIndex = toNode === null || isDropInsideItem ? toItems.length : toItems.findIndex((item) => item.id === toNode.itemData.id);
