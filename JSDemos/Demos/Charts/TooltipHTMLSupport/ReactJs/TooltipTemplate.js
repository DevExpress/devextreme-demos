...
export default function TooltipTemplate(info) {
  return (
    <div className="state-tooltip">
      <img src={getImagePath(info.point)} />
      ...
      <div>
        ...
        <span className="area-km">{formatNumber(info.point.data.area)}</span> km<sup>2</sup> (
        <span className="area-mi">{formatNumber(0.3861 * info.point.data.area)}</span> mi
        <sup>2</sup>)
      </div>
    </div>
  );
