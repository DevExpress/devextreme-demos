...
1. **Apply the same changes to the component's data source**       
If the server successfully saves changes, call the **applyChanges** method to save the same changes in the component's data source (see the `updateOrders` method in `app.service.ts`).
