...
1. **Apply the same changes to the component's data source and reset edit state**       
If the server successfully saves changes, call the **applyChanges** method to save the same changes in the component's data source. Assign **null** to the **editRowKey** and an empty array to the **changes** option. This resets edit state (see the `SAVING_SUCCESS` handler in `reducer.js`).
