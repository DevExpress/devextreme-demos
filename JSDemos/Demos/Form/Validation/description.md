...
This demo also shows an implementation of the **Reset** button. When users click the button, it resets all Form data to initial values. The Form also uses the [isDirty](/Documentation/ApiReference/UI_Components/dxForm/Configuration/#isDirty) flag to check whether changes are made, and if not, disables the button.