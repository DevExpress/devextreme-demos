...
['jQuery', 'React', 'Vue', 'Angular'].forEach((approach) => {
  ...
  getDemoPaths(approach).forEach((demoPath, index) => {
    ...
    runTestAtPage(test, `http://127.0.0.1:808${getPortByIndex(index)}/JSDemos/Demos/${widgetName}/${demoName}/${approach}/dx.fluent.blue.light.css`)
      .clientScripts(clientScriptSource)(testName, async (t) => {
        ...
        if (process.env.STRATEGY === 'accessibility') {
          ...
          const comparisonResult = await compareScreenshot(t, `${testName} (fluent.blue.light).png`, undefined, comparisonOptions);
