...
runManualTest('PivotGrid', 'IntegratedFieldChooser', ['jQuery', 'React', 'Vue', 'Angular'], (test) => {
  test('IntegratedFieldChooser', async (t) => {
    ...
    await testScreenshot(t, takeScreenshot, 'Integrated field chooser.png', { element: `.${FIELD_CHOOSER_CONTENT}` });

    const textEditorInput = Selector(`.${TREEVIEW_SEARCH_FIELD} .${TEXTEDITOR_INPUT}`);
    await t
      .typeText(textEditorInput, 'Product M');

    await testScreenshot(t, takeScreenshot, 'Integrated field chooser after search of hierarchy field.png', { element: `.${FIELD_CHOOSER_CONTENT}` });
