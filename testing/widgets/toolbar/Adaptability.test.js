...
runManualTest('Toolbar', 'Adaptability', ['jQuery', 'React', 'Vue', 'Angular'], (test) => {
  test('Adaptability', async (t) => {
    ...
    await takeScreenshot(`toolbar_multiline_mode_minimize(${t.ctx.theme}).png`);

    await t.click($(`.${OPTIONS_CONTAINER_CLASS} .${CHECKBOX_CLASS}`));
    await takeScreenshot(`toolbar_singleline_mode_init(${t.ctx.theme}).png`);

    await t.click($(`.${TOOLBAR_CLASS} .${DROP_DOWN_MENU_BUTTON_CLASS}`));
    await takeScreenshot(`toolbar_singleline_mode_menu_open(${t.ctx.theme}).png`);
