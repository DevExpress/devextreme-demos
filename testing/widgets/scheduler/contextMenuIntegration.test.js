...
runManualTest('Scheduler', 'ContextMenuIntegration', 'jQuery', (test) => {
  test('ContextMenuIntegration', async (t) => {
    ...
    await takeScreenshot(`scheduler_contextMenu_init(${t.ctx.theme}).png`);

    await t
      .rightClick('.dx-scheduler-appointment');
    await takeScreenshot(`scheduler_contextMenu_click(${t.ctx.theme}).png`);

    await t
      .rightClick(Selector('.dx-scheduler-date-table-cell').nth(6));
    await takeScreenshot(`scheduler_contextMenu_cell_click(${t.ctx.theme}).png`);

    await t
      .rightClick(Selector('.dx-scheduler-header-panel-cell').nth(2));
    await takeScreenshot(`scheduler_contextMenu_header_click(${t.ctx.theme}).png`);
